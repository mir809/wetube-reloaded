...
const selectProfile = "select";
const selectManage = "select";
const selectUserInfo = "select";
const selectPassChange = "select";
const selectDelete = "select";
// 계정 관리 페이지 사이드바

const selectChannelHome = "select";
const selectChannelInfo = "select";
// 유저 채널 표시
...
export const finishGithubLogin = async (req, res) => {
  ...
  if ("access_token" in tokenRequest) {
    ...
    req.flash(
      "success",
      `${user.name}님, 환영합니다. (Github 로그인 계정입니다.)`
    );
...
export const channel = async (req, res) => {
  ...
  if (!user) {
    ...
    return res
      .status(404)
      .render("404", { pageTitle: "User not found", noneUser });
  }

  req.session.beforeSmallPlayer = "/users" + req.url;
  // 돌아갈 주소 저장
...
export const channelInfo = async (req, res) => {
  const { id } = req.params;
  const user = await User.findById(id).populate("videos");
  if (!user) {
  ...
  // 회원가입일(joinAt)을 특정 형식으로 변환하여 표시
  const joinDate = user.joinAt.toLocaleDateString("ko-KR", {
    year: "numeric",
    month: "2-digit",
    day: "2-digit",
  });
  ...
  for (const video of user.videos) {
    totalViews += video.meta.views;
  }

  return res.render("users/channel-info", {
    pageTitle: `${user.name} - NewTube`,
    user,
    id,
    selectChannelInfo,
...
export const postProfileEdit = async (req, res) => {
  ...
  req.flash("info", "프로필 변경사항이 저장되었습니다.");
...
export const postAccountEdit = async (req, res) => {
  ...
  req.flash("info", "계정 변경사항이 저장되었습니다.");
...
export const themeChange = async (req, res) => {
  const {
    params: { id },
  } = req;

  req.session.Theme = id;
